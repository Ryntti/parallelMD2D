...
void exchange_boundary_data(std::vector<vec2D> &local_pos, int num_cols, MPI_Datatype Vec2D_Type, MPI_Datatype Col_Type, MPI_Comm comm_2d) {
    /* Exchange back and forth boundary atom data between neighboring processes in both axes of communication */
    int axis = 0;   // vertical axis message passing first
    // Get neighboring ranks
    int north, south; // the ranks of neighboring processes (in the 2d cart topology) are saved for message passing
...
int main(int argc, char** argv) {
    ...
    // define custom MPI types for vec2Ds and columns of a 2D array of vec2D structs
